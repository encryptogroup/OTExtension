...
class KKOTExtSnd : public OTExtSnd {

public:
	KKOTExtSnd(uint32_t nSndVals, crypto* crypt, RcvThread* rcvthread, SndThread* sndthread) {
		uint32_t numbaseots = max((uint64_t) 2*crypt->get_seclvl().symbits, pad_to_power_of_two(nSndVals));
		cout << "Using KK OT extension sender" << endl;
		...
		InitSnd(nSndVals, crypt, rcvthread, sndthread, 2*crypt->get_seclvl().symbits);
