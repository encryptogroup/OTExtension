...
#include "oos-ot-ext-snd.h"

BOOL OOSOTExtSnd::sender_routine(uint32_t id, uint64_t myNumOTs) {
...
//	uint32_t choicecodebitlen = ceil_log2(m_nint_sndvals);
	...
	if (!OOSCheck(Q_list, chan, ceil_divide(myNumOTs, diff_choicecodes), myStartPos1ooN)) {
		cout << "OOS: invalid checksum." << endl;
		exit(-1);
	}
...
void OOSOTExtSnd::ComputeBaseOTs(field_type ftype) {
...
void OOSOTExtSnd::KKHashValues(CBitVector& Q, CBitVector* seedbuf, CBitVector* snd_buf, uint64_t OT_ptr, uint64_t OT_len, uint64_t** mat_mul) {
...
void OOSOTExtSnd::KKMaskAndSend(CBitVector* snd_buf, uint64_t OT_ptr, uint64_t OT_len, channel* chan) {
...
bool OOSOTExtSnd::OOSCheck(vector<CBitVector *> &Q_list, channel *chan, uint64_t internal_numOTs, uint64_t firstOTid) {
	...
	// STEP 2 - GENERATE WEIGHTS (and send seed to receiver)
	...
	// STEP 3 - CALCULATE CHECKSUM (and verify it)
	CBitVector qChecksums(8 * m_nCodeWordBytes * m_nAdditionalOTs);
	uint64_t wd_size_bytes = m_nBlockSizeBytes;
	for (uint32_t i = 0; i < m_nAdditionalOTs; i++) {
		qChecksums.SetBytes(Q.GetArr() + i * wd_size_bytes, i * m_nCodeWordBytes, m_nCodeWordBytes);
	}

	uint64_t otid = 0;
	uint64_t vChoicesOffset = 0;
	for (size_t i = 0; i < Q_list.size(); i++) {
		CBitVector *Q_i = Q_list[i];
		...
		for (uint64_t j = 0; j < NUMOTBLOCKS * wd_size_bits; j++) {
			...
			for (uint32_t checkIdx = 0; checkIdx < m_nAdditionalOTs; checkIdx++) {
				...
				if (randomBits.GetBit(checkIdx * internal_numOTs + otid)) {
					qChecksums.XORBytes(Q_i->GetArr() + j*wd_size_bytes, checkIdx * m_nCodeWordBytes, m_nCodeWordBytes);
				}
			}
	...
	// receive all checksums
	uint8_t *tChecksums_buffer = chan->blocking_receive();
	uint8_t *choicesChecksums_buffer = chan->blocking_receive();
	CBitVector tChecksums;
	CBitVector choicesChecksums;
	tChecksums.AttachBuf(tChecksums_buffer, m_nCodeWordBytes * m_nAdditionalOTs);
	choicesChecksums.AttachBuf(choicesChecksums_buffer, bits_in_bytes(choicecodebitlen * m_nAdditionalOTs));


	// for each checksum, first we calculate C(w) AND b (in OOS16 terminology)
	// that is codeword for the choice-checksum AND baseOT-choices
	// then we add t-checksum and q-checksum - the result should be zero
	CBitVector mask(m_nCodeWordBits);
	for (uint32_t checkIdx = 0; checkIdx < m_nAdditionalOTs; checkIdx++) {
		mask.Copy(m_tBaseOTChoices.front()->GetArr(), 0, m_nCodeWordBytes);
		uint32_t choice = choicesChecksums.Get<uint32_t>(checkIdx * choicecodebitlen, choicecodebitlen);
		mask.ANDBytes((uint8_t*) m_vCodeWords[choice], 0, m_nCodeWordBytes);
		mask.XORBytes(tChecksums.GetArr() + checkIdx * m_nCodeWordBytes, m_nCodeWordBytes);
		mask.XORBytes(qChecksums.GetArr() + checkIdx * m_nCodeWordBytes, m_nCodeWordBytes);
		...
		// no need to reset mask - it is already all-zeros
	}
