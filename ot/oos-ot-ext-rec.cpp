...
#include "oos-ot-ext-rec.h"


BOOL OOSOTExtRec::receiver_routine(uint32_t id, uint64_t myNumOTs) {
	// differences to KK:
	...
	// In between the loops some additional OTs must be handled and checksums must be calculated and sent to the sender.
	// Note that for checksum calculation a seed from the server is required that will only be provided after the first
	// phase is completed. (That means no checksum calculation inside the first loop)
	...
	OOSCheck(T_list, chan, ceil_divide(myNumOTs, diff_choicecodes), myStartPos1ooN);
...
void OOSOTExtRec::GenerateChoiceCodes(CBitVector* choicecodes, CBitVector* vSnd, CBitVector* T, uint32_t startpos, uint32_t len) {
	uint32_t tmpchoice;
...
void OOSOTExtRec::KKHashValues(CBitVector* T, CBitVector* seedbuf, CBitVector* maskbuf, uint64_t OT_ptr, uint64_t OT_len, uint64_t** mat_mul) {
...
void OOSOTExtRec::KKSetOutput(CBitVector* maskbuf, uint64_t otid, uint64_t otlen, queue<mask_block*>* mask_queue,
		channel* chan) {
...
void OOSOTExtRec::KKReceiveAndUnMask(channel* chan, queue<mask_block*>* mask_queue) {
...
void OOSOTExtRec::KKMaskBaseOTs(CBitVector* T, CBitVector* SndBuf, uint64_t numblocks) {
	uint64_t rowbytelen = m_nBlockSizeBytes * numblocks;
	SndBuf->XORBytes(T->GetArr(), 0, rowbytelen * m_nBaseOTs);
}


void OOSOTExtRec::ComputeBaseOTs(field_type ftype) {
...
void OOSOTExtRec::OOSCheck(vector<CBitVector *> &T_list, channel *chan, uint64_t internal_numOTs, uint64_t firstOTid) {
	// STEP 1 - ADDITIONAL OTs
	...
	uint32_t choicecodebitlen = ceil_log2(m_nint_sndvals);
	CBitVector additionalChoices(m_nAdditionalOTs * choicecodebitlen, m_cCrypt);
	...
	// this is a little hack. GenerateChoiceCodes() unfortunately only works on m_vChoices and not on any arbitrary
	// choice vector - so we temporarily swap m_vChoices for the additionalChoices vector
	CBitVector *tmp_vChoices = m_vChoices;
	m_vChoices = &additionalChoices;
	GenerateChoiceCodes(&choicecodes, &vSnd, &T, 0, m_nAdditionalOTs);
	m_vChoices = tmp_vChoices;

	KKMaskBaseOTs(&T, &vSnd, additionalOTBlocks);
	T.Transpose(wd_size_bits, OTsInMatrix);
	SendMasks(&vSnd, chan, additionalID, OTsInMatrix, 0);

	// STEP 2 - GENERATE WEIGHTS (from server seed)

	// receive seed from server and init random state
	...
	// In terms of OO16: we need s random strings of size m
	// (i.e. we need m_nAdditionalOT random strings of size internal_numOTs)
	uint32_t randomBits_byteSize = bits_in_bytes(m_nAdditionalOTs * internal_numOTs);
	uint8_t *randomBits_buffer = (uint8_t *)malloc(randomBits_byteSize);
	gen_rnd_bytes(&weights_prf_state, randomBits_buffer, randomBits_byteSize);
	CBitVector randomBits;
	randomBits.AttachBuf(randomBits_buffer, randomBits_byteSize);


	// STEP 3 - CALCULATE CHECKSUM (and send it)

	// for the choices-checksums we can just build the checksum on top of the choices in additionalChoices
	// (because they need to be added to the checksums anyways)
	// But for the t-checksums the dimensions of T might not be perfect, so we move the rows of T into a new vector
	CBitVector tChecksums(8 * m_nCodeWordBytes * m_nAdditionalOTs);
	uint64_t wd_size_bytes = m_nBlockSizeBytes;
	for (uint32_t i = 0; i < m_nAdditionalOTs; i++) {
		tChecksums.SetBytes(T.GetArr() + i * wd_size_bytes, i * m_nCodeWordBytes, m_nCodeWordBytes);
	}

	uint64_t otid = 0;
	uint64_t vChoicesOffset = 0;
	for (size_t i = 0; i < T_list.size(); i++) {
		CBitVector *T_i = T_list[i];
		// note: each T contains NUMOTBLOCKS * wd_size_bits OTs - except the last one (which we handle with break)
		for (uint64_t j = 0; j < NUMOTBLOCKS * wd_size_bits; j++) {
			if (otid >= internal_numOTs)
				break;

			uint32_t choice = m_vChoices->Get<uint32_t>(vChoicesOffset, choicecodebitlen);
			// remember: there are m_nAdditionalOTs checksums
			for (uint32_t checkIdx = 0; checkIdx < m_nAdditionalOTs; checkIdx++) {
				//if (randomBits.GetBit(otid * m_nAdditionalOTs + checkIdx)) {
				if (randomBits.GetBit(checkIdx * internal_numOTs + otid)) {
					tChecksums.XORBytes(T_i->GetArr() + j*wd_size_bytes, checkIdx * m_nCodeWordBytes, m_nCodeWordBytes);
					additionalChoices.XOR(choice, checkIdx * choicecodebitlen, choicecodebitlen);
				}
			}

			vChoicesOffset += choicecodebitlen;
			otid++;
		}
	}

	chan->send(tChecksums.GetArr(), m_nCodeWordBytes * m_nAdditionalOTs);
	chan->send(additionalChoices.GetArr(), bits_in_bytes(m_nAdditionalOTs * choicecodebitlen));
}
