...
rcv_check_t ALSZOTExtRec::EnqueueSeed(uint8_t* T0, uint8_t* T1, uint64_t otid, uint64_t numblocks) {
...
void ALSZOTExtRec::ComputeOWF(queue<rcv_check_t>* check_buf_q, channel* check_chan) {//linking_t* permbits, int nchecks, int otid, int processedOTs, BYTE* outhashes) {
	...
	rcv_check_t check_buf = check_buf_q->front();
	check_buf_q->pop();
